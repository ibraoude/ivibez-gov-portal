...
export function LocalForm({ value, onChange, onCaptchaChange }: Props) {
  ...
  // v3 token state (optional: lift this to parent and remove from here)
  ...
  /* -------------------------
     SAFE FIELD SETTER
  ------------------------- */
  function setField<K extends keyof LocalFormData>(key: K, val: LocalFormData[K]) {
    onChange({ ...v, [key]: val });
  }
  ...
  async function handleCaptcha() {
    try {
      ...
      // Use a meaningful action label; match it on the server if you want
      const token = await getRecaptchaToken("local_form_review");
