...
type Props = {
  ...
  /** Optional: if the parent wants to receive the v3 token */
...
export function StateForm({ value, onChange, onCaptchaChange }: Props) {
  ...
  /* ---------- SAFE FIELD SETTER ---------- */
  function setField<K extends keyof StateFormData>(key: K, val: StateFormData[K]) {
    onChange({ ...v, [key]: val });
  }
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                const token = await getRecaptchaToken("state_form_review");
...
function ArraySection({ title, items, selected, toggle }: ArraySectionProps) {
  return (
    <div className="mt-6">
      {title && <p className="mb-2 text-sm font-medium text-gray-900">{title}</p>}
