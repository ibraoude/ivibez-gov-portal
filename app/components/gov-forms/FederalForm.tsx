...
export function FederalForm({ value, onChange, onCaptchaChange }: Props) {
  ...
  // v3 token state (optional – you can lift this to the parent and remove from here)
  const [captchaToken, setCaptchaToken] = useState<string | null>(null);
  const [captchaBusy, setCaptchaBusy] = useState(false);
  const [captchaError, setCaptchaError] = useState<string | null>(null);

  function setField<K extends keyof FederalFormData>(key: K, val: FederalFormData[K]) {
    onChange({ ...v, [key]: val });
  }
  ...
  async function handleCaptcha() {
    try {
      ...
      // Choose an action; you can check it on the server if desired
      const token = await getRecaptchaToken("federal_form_review");
      setCaptchaToken(token);
      onCaptchaChange?.(token);
    } catch {
      setCaptchaError("Security system is not ready. Please try again.");
    } finally {
  ...
  return (
    <div className="space-y-10">
      ...
      <Section title="SECTION 3: PROCUREMENT DETAILS">
        <Grid>
          <Select
            label="Is funding currently allocated?"
      ...
      <Section title="SECTION 5: COMPLIANCE & SECURITY">
        <Grid>
          <Select
            label="Does this involve sensitive/classified data?"
      ...
      <Section title="SECTION 6: EVALUATION & PERFORMANCE">
        ...
        <div className="mt-8 flex items-center gap-3">
          ...
          >
            {captchaBusy ? "Requesting token..." : (captchaToken ? "Refresh security token" : "Verify security")}
          </button>

          {captchaToken && <span className="text-xs text-green-700">Security token ready</span>}
          {captchaError && <span className="text-xs text-red-600">{captchaError}</span>}
        </div>
      </Section>
    </div>
  );
