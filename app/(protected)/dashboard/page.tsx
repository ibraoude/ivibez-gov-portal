...
export default function Dashboard() {
  ...
  const atRiskContracts = useMemo(
    () => contracts.filter((c) => isAutoAtRisk(c)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [contracts]
  );

  const portfolioValue = useMemo(
    () => contracts.reduce((s, c) => s + (c.final_amount || 0), 0),
    [contracts]
  );
  const activeExposure = useMemo(
    () => activeContracts.reduce((s, c) => s + (c.final_amount || 0), 0),
    [activeContracts]
  );
  const atRiskRevenue = useMemo(
    () => atRiskContracts.reduce((s, c) => s + (c.final_amount || 0), 0),
    [atRiskContracts]
  );
  const avgCompletion = useMemo(() => {
    ...
    return Math.round(sum / activeContracts.length);
  }, [activeContracts]);

  /* ===================== Sparklines =====================
   * We’ll aggregate last 12 weeks by creation date and show cumulative totals.
   */
  const sparkWeeks = 12;
  const sparklineData = useMemo(() => {
    ...
    for (let i = sparkWeeks - 1; i >= 0; i--) {
      ...
      const inWeek = (c: GovernmentContract) => {
        const created = new Date(c.created_at);
        return created >= start && created < endWeek;
      };

      const weekContracts = contracts.filter(inWeek);
      const weekActive = weekContracts.filter((c) => c.status === "active");
      const weekRisk = weekContracts.filter((c) => isAutoAtRisk(c));

      const portfolio = weekContracts.reduce((s, c) => s + (c.final_amount || 0), 0);
      const active = weekActive.reduce((s, c) => s + (c.final_amount || 0), 0);
      const risk = weekRisk.reduce((s, c) => s + (c.final_amount || 0), 0);
      const avg =
        weekActive.length === 0
          ? 0
          : weekActive.reduce((s, c) => s + (c.progress_percentage || 0), 0) /
            weekActive.length;
    ...
    // make cumulative for money metrics so sparkline looks nice
    let cumP = 0,
      cumA = 0,
      cumR = 0;
    return weeks.map((w) => {
                            ...
                            {burn && (
                              <div className="mt-2 text-xs space-y-1 text-gray-700 dark:text-gray-200">
                                <FlexRow
                                  ...
                                  value={`$${burn.earnedRevenue.toLocaleString()}`}
                                />
                                <FlexRow
                                  label="Expected by Now"
                                  value={`$${burn.expectedRevenue.toLocaleString()}`}
                                />
                                <FlexRow
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="animate-pulse border border-gray-100 dark:border-white/10 rounded-xl p-4 bg-white dark:bg-white/5"
        >
...
/** StatCard with sparkline + animated number + optional ring */
function StatCard({
  title,
  value,
  icon,
  ...
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="relative bg-white dark:bg-white/5 rounded-xl shadow-md p-5 overflow-hidden group border border-gray-100 dark:border-white/10"
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-60 group-hover:opacity-80 transition pointer-events-none`}
      />
      <div className="relative flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <AnimatedNumber
            value={value}
            formatter={(n) =>
              isPercent ? `${n.toFixed(0)}%` : money ? `$${Math.round(n).toLocaleString()}` : `${Math.round(n)}`
            }
...
/** Animated number using Framer Motion's animate() */
